...
router.post('/add', auth.authenticateToken, checkRole.checkRole, function (req, res, next) {
  var category = req.body;
  query = "insert into category(name) values(?)";
  connection.query(query, [category.name], function (err, results) {
    if (!err) {
...
router.patch('/update', auth.authenticateToken, checkRole.checkRole, function (req, res, next) {
  ...
  connection.query(query, [product.name, product.id], function (err, results) {
    if (!err) {
